...
val init : ?resizer:(int -> int -> int) -> int -> int -> 'a -> (int -> 'a) -> 'a xarray_t
...
val delete : 'a xarray_t -> int -> unit
...
val of_array : ?resizer:(int -> int -> int) -> 'a -> 'a array -> 'a xarray_t
(** [of_array null arr] returns an xarray with the elements of [arr] in it
    in order, and [null] as it's null element.  A resizing strategy can
    be specified- the default is to use [exponential_resizer]. *)

val copy : ?resizer:(int -> int -> int) -> 'a xarray_t -> 'a xarray_t
...
val sub : ?resizer:(int -> int -> int) -> 'a xarray_t -> int -> int -> 'a xarray_t
...
val mapi : ?resizer:(int -> int -> int) -> (int -> 'a -> 'b) -> 'b -> 'a xarray_t -> 'b xarray_t

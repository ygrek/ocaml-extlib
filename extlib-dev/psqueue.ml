...
module Make (Ord: OrderedPriorityType) = struct
    type key_t = Ord.k_t
    ...
    type psq_ltree_t
         ...
         | Loser of color_t       (* node color *)
                  ...
                  * elem_t        (* loser data *)
                  * key_t         (* search key *)
                  * psq_ltree_t   (* left subtree *)
                  * psq_ltree_t   (* right subtree *)
    ...
    type  psq_t
          = Void
          | Winner of key_t          (* winner key *)
                    * elem_t         (* winner data *)
                    * psq_ltree_t    (* loser (pennant) tree *)
                    * key_t          (* max search key *)
        ...
        let rotate_left a b col =
            match (a, b) with
                ...
                | (Loser(_, Left, a_key, a_data, a_skey, c, _),
                   Loser(_, Left, b_key, b_data, b_skey, d, e))
                ->
                    if ((Ord.pricmp a_data b_data) < 0) then
                        (* swap required *)
    ...
    let play c a b =
       match (a, b) with
           ...
           | (Winner(b_key, b_data, t, m),
               ...
               *)
              if ((Ord.pricmp b_data b'_data) <= 0) then
                  Winner(b_key, b_data,
                         (balance (Loser(c, Right, b'_key,
                                                 b'_data, m, t, t'))),
                         m')
              else
                  Winner(b'_key, b'_data,
                         (balance (Loser(c, Left, b_key, b_data,
                                                 m, t, t'))),
                         m')
    ...
    let search found notfound empty k q =
        let rec search_int q =
            match q with
                Start -> notfound ()
                | Loser(_, _, k', data, skey, l, r) ->
                    if ((Ord.keycmp k' k) == 0) then
                        found data
                    else
                        if ((Ord.keycmp k' skey) <= 0) then search_int l
                        else search_int r
        in
        match q with
            Void -> empty ()
            | Winner(k', data, ltree, max) ->
                if ((Ord.keycmp k' k) == 0) then
                    found data
                else if ((Ord.keycmp k' max) <= 0) then
                    search_int ltree
                else
                    notfound ()
                    ...
                    if ((Ord.keycmp k b_key) == 0) then
                        ...
                        begin
                            ...
                            in match (b') with
                               Void -> t
                               | Winner(b'_key, _, _, _) ->
                                   if ((Ord.keycmp b'_key b_key) < 0) then
                                       play Red b' t
                                   else
                                       play Red t b'
                        end
                | Many(c, tl, tr) ->
                    if ((Ord.keycmp k (max_key tl)) <= 0) then
                        play c (modify_int tl) tr
                    else
                        play c tl (modify_int tr)
        in
            match q with
                Void -> empty ()
                | _ -> finalize (modify_int q)
    ...
    let find k q =
        ...
        in
            search found notfound empty k q
    ...
    let contains k q =
        ...
        and notfound () = false
        and empty () = false
        in search found notfound empty k q
        ...
        let rec loser2str l =
            ...
            match l with
               ...
               | Loser(col, dom, key, data, skey, left, right) ->
                   "Loser(" ^ (col2str col) ^ ", " ^ (dom2str dom) ^ ", " ^
                   (key2str key) ^ ", " ^ (data2str data) ^
                   ", " ^ (key2str skey) ^ ", " ^ (loser2str left) ^ ", " ^
                   (loser2str right) ^ ")"
        in
        match q with
            Void -> "Void"
    ...
    let to_pri_list q = 
        let rec pri_merge a b accum =
            match (a, b) with
                ...
                ->
                    if ((Ord.pricmp aelem belem) <= 0) then

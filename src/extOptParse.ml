...
module Opt =
  struct
    ...
    let is_set opt = ExtOption.is_some (opt.option_get ())
...
module OptParser =
  struct
    ...
    type group = { 
      ...
      og_options :
        ((char list * string list) * string list * string option) ExtRefList.t;
      og_children : group ExtRefList.t 
    }
    ...
    type t = { 
      ...
      op_long_options : GetOpt.long_opt ExtRefList.t;
      op_short_options : GetOpt.short_opt ExtRefList.t;
      ...
      if lnames = [] && snames = [] then
        ...
        let snames' =
          List.fold_left (fun r (x, _, _) -> x :: r) []
            (ExtRefList.to_list optparser.op_short_options)
        and lnames' =
          List.fold_left (fun r (x, _, _) -> x :: r) []
            (ExtRefList.to_list optparser.op_long_options)
        in
        let sconf =
        ...
        (* Add to display list. *)
        if not hide then
          ExtRefList.add group.og_options
        ...
        let nargs = List.length opt.option_metavars in
          List.iter
            (fun short ->
               ExtRefList.add optparser.op_short_options
               (short, nargs, opt.option_set))
            snames;
          List.iter
            (fun long ->
               ExtRefList.add optparser.op_long_options
      ...
      let g =
        {
          ...
          og_options = ExtRefList.empty (); 
          og_children = ExtRefList.empty ()
        }
      in
      ExtRefList.add parent.og_children g; g
    ...
    let make ?(usage = "%prog [options]") ?(status = 1) ?description ?version
      ...
      let optparser =
        {
          ...
          op_prog = ExtOption.default (Filename.basename Sys.argv.(0)) prog;
          op_formatter = formatter; 
          op_short_options = ExtRefList.empty ();
          op_long_options = ExtRefList.empty ();
          op_groups = {
            og_heading = "options"; 
            og_options = ExtRefList.empty ();
            og_children = ExtRefList.empty ();
      ...
      in
      ExtOption.may                         (* Add version option? *)
        (fun version ->
    ...
    let error optparser ?(chn = stderr) ?status message =
      fprintf chn "%s%s: %s\n" (format_usage optparser "\n") optparser.op_prog
        message;
      flush chn;
      exit (ExtOption.default optparser.op_status status)
    ...
    let usage optparser ?(chn = stdout) () =
      let rec loop g =
        ...
        (* Child groups: *)
        output_string chn "\n";
        ExtRefList.iter loop g.og_children;
    ...
    let parse optparser ?(first = 0) ?last argv =
      ...
      begin 
        try
          GetOpt.parse (ExtRefList.push args)
            (GetOpt.find_short_opt
               (ExtRefList.to_list optparser.op_short_options))
            (GetOpt.find_long_opt (ExtRefList.to_list optparser.op_long_options))
      ...
      List.rev (ExtRefList.to_list args)
